...
@Component({
  ...
  templateUrl: './add-barang-rusak.component.html',
  styleUrl: './add-barang-rusak.component.scss'
})
export class AddBarangRusakComponent implements OnInit {

    BarangRusakForm!: FormGroup;
    isLoading = false;
    divisiList: any[] = [];
    userList: any[] = [];
    ...
    constructor(
      ...
      public dialogRef: MatDialogRef<AddBarangRusakComponent>,
    ...
    ngOnInit(): void {
        ...
        this.sebaranBarang = this.data;

        // Initialize form controls - hanya field yang diperlukan
        this.BarangRusakForm = this.fb.group({
          qty_barang: [null, [Validators.required, Validators.min(1), Validators.max(this.sebaranBarang?.qty_barang)]],
          posisi_awal: [this.sebaranBarang?.posisi_awal || ''],
          posisi_akhir: [null, ],
          catatan: [null,]
        });
   ...
   try {
    ...
    preventInvalidChars(event: any) {
        ...
        const maxQty = this.sebaranBarang?.qty_tersedia;
